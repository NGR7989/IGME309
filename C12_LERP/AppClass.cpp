...
void Application::Display(void)
{
	...
	*/
	...
	v3Start = points[route]; //start at the current route
	v3End = points[(route + 1) % points.size()]; //end at route +1 (if overboard will restart from 0)

	//get the percentace
	float fTimeBetweenStops = 2.0;//in seconds
	//map the value to be between 0.0 and 1.0
	float lerp = MapValue(fTimer, 0.0f, fTimeBetweenStops, 0.0f, 1.0f);

	//calculate the current position
	vector3 v3CurrentPos = glm::lerp(v3Start, v3End, lerp);
	...
	if (lerp >= 1.0f)
	{
		...
		route %= points.size();//make sure we are within boundries
	}
	...
	{
		m_pModelMngr->AddSphereToRenderList(glm::translate(points[i]) * glm::scale(vector3(0.15f)), C_RED, RENDER_WIRE);
	}
